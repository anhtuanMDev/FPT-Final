...
let createUsers = (num) => {
    let query = "INSERT INTO users ( `Id`,`Name`,`Email`,`Password`,`Rank`,`Status`,`CreateAt`,`UpdateAt`,`DeleteAt`) VALUES";
    ...
    for (let i = 0; i < num; i++) {
        ...
        const status = Math.floor(Math.random() * 2) === 0 ? 'Active' : 'Inactive';
        const date = randomDate();
        const createAt = formatDate(date);
        const updateAt = formatDate(conditionDate(date));
        query = query.concat(`('${id}','${name}','${email}','123456',${Math.floor(Math.random() * 2000)}, '${status}', '${createAt}', '${updateAt}', NULL)`);
