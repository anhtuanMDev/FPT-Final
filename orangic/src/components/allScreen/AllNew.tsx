...
const AllNew = () => {
  const navigation = useNavigation<NavigationProp<ParamList, 'AllNew'>>();
  ...
  const getFoods = async () => {
    const response = await AxiosInstance().post('/get-20-new-foods.php', {
      id: userID,
    });
  ...
  return (
    <View style={[screenStyles.parent_container]}>
      <Text
        ...
        {`Tất cả món ăn mới nổi tiếng (${data.length})`}
      </Text>
      <FlatList
        data={data}
        numColumns={2}
