...
const AllPopular = () => {
  const navigation = useNavigation<NavigationProp<ParamList, 'AllPopular'>>();
  ...
  const getFoods = async () => {
    const response = await AxiosInstance().post('/get-20-popular-foods.php', {
      id: userID,
    });
  ...
  return (
    <View style={[screenStyles.parent_container]}>
      <Text
        ...
        {`Tất cả món ăn nổi tiếng (${data.length})`}
      </Text>
      <FlatList
        data={data}
        numColumns={2}
