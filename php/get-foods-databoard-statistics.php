...
try {
    ...
    switch ($timeLine) {
        case "Today":
            foreach ($timePoints as $key => $value) {
                ...
                AND YEAR(CreateAt) = YEAR(CURDATE()) THEN 1 END) as Food
                FROM foods 
        ...
        case 'This Month':
            foreach ($timePoints as $key => $value) {
                ...
                AND MONTH(CreateAt) = MONTH(CURDATE())
                AND YEAR(CreateAt) = YEAR(CURDATE()) THEN 1 ELSE 0 END) as Food
                FROM foods 
                ORDER BY `CreateAt` DESC";
                $stmt = $dbConn->prepare($query);
                $stmt->execute();
    ...
    $min = min(min($output['foods']), min($output['revenues']), min($output['banneds']));
    $max = max(max($output['foods']), max($output['revenues']), max($output['banneds'])) + 10;
