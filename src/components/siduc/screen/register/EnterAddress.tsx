...
import { selectAddressFulfilled, selectAddressPending } from '../../../../helper/state/appTab/addressSlice';
...
const EnterAddress = () => {
  const [address, dispatchAddress] = useReducer(handleAddress, {
    city: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
    district: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
    ward: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
    phone: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
    userAddress: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
  });
  ...
  const handleAdressUpload = async (adr: AddressState, id : string) => {
    try{
      ...
      if(!phone || !userAddress || !city || !district || !ward) throw new Error('Chưa điền đầy đủ thông tin');
      ...
      console.warn('handleAddress', err)
    }
  };
